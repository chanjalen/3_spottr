...
export default function OnboardingStep3Screen({ navigation }: Props) {
  ...
  const advance = async (data: { phone_number?: string; skip_phone?: boolean }) => {
    setLoading(true);
    setError(null);
    try {
      const { user } = await apiUpdateOnboarding(data);
  ...
  const handleContinue = () => advance({ phone_number: phone.trim() });
  const handleSkip = () => advance({ skip_phone: true });
    ...
    >
      ...
      >
        ...
        <Text style={styles.subheading}>
          Used for account recovery and optional 2-factor auth.{'\n'}
        ...
        <Pressable onPress={handleSkip} disabled={loading} style={styles.skipBtn}>
